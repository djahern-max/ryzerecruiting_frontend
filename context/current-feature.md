...
- 2026-07-26 — Implemented in `Landing.jsx` + `Landing.module.css`: `VideoSection` filled in (click-to-play `<video>` with poster, `preload="metadata"`, `playsInline`, custom play-button affordance, `.videoFrame` aspect-ratio-16/9 box reusing `.previewWrap`/`.previewGlow` framing); `demo_video_played` fires via `posthog.capture` in `onPlay`, guarded by `playedOnceRef` (useRef) so it never re-fires on pause/resume/replay; `DemoChat` relocated to a new `.chatSection` below the proof row with a light "See RYZE Intelligence in action" heading. Verified via headless Chromium (Playwright, driven directly since `chromium-cli` wasn't installed): `npm run build` passes clean; desktop + iPhone-13 viewport screenshots confirm no layout shift, no horizontal overflow, no console errors; CDP network trace confirms only the poster (~149KB) loads on initial page load and the video's `preload="metadata"` request does not pull the full ~134MB file; play button correctly starts playback (`video.paused` false, native controls shown) on both desktop click and mobile tap. PostHog capture itself couldn't be observed end-to-end because dev mode calls `opt_out_capturing()` (per `main.jsx`) so no network/`window.posthog` hook is available in dev — verified the once-only guard by code review instead (ref persists across repeated `onPlay` events, matching the spec). Both hero CTAs (`/signup`, `/demo`) unchanged. Feature appears complete against all acceptance criteria — pending your review/confirmation before archiving.
- (append per session)
