...
// ---------------------------------------------------------------------------
// CandidateMatchRow — compact builder match row shown inside an opportunity card
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// JobCard — AI implementation opportunity with embedded builder matches
// ---------------------------------------------------------------------------
  ...
  return (
    <div className={styles.jobCard}>
      ...
      <div className={styles.jobCardHeader}>
        ...
        <span className={styles.jobOpenBadge}>Active</span>
      </div>

      {/* ── Implementation need / requirements ── */}
      {job.requirements && (
      ...
      <div className={styles.candidateSection}>
        ...
        {matchesLoading ? (
          <div className={styles.candidatesLoading}>
            ...
            <span>Matching builders to this opportunity…</span>
          </div>
        ) : !hasCandidates ? (
          <div className={styles.candidatesEmpty}>
            ...
            <span>
              {candidateMatches === null
                ...
                : 'No builders indexed for this opportunity yet — check back soon.'}
            </span>
          </div>
        ) : showCandidates ? (
          <>
            ...
            <div className={styles.candidateCta}>
              <button className={styles.requestBtn} onClick={onSchedule}>
                Talk to RYZE about this AI opportunity →
              </button>
            </div>
          </>
        ) : null}
...
export default function EmployerDashboard() {
  ...
  // EP15: candidateMatches is a map: { [job_order_id]: builders/candidates[] | null }
  ...
  // EP15: Fetch builder/candidate matches for each employer-linked opportunity once roles load
  useEffect(() => {
    ...
    // Fetch builder/candidate matches for all opportunity records in parallel
    Promise.all(
      myRoles.map(job =>
  ...
  // Split opportunity records: employer's AI opportunities vs other network opportunities
  ...
  return (
    <div className={styles.page}>
      ...
      <main className={styles.main}>
        ...
        {/* ── Your AI Opportunities (with builder matches) ── */}
        {!rolesLoading && myRoles.length > 0 && (
          <section className={styles.section}>
            <div className={styles.sectionHeader}>
              <h3 className={styles.sectionTitle}>Your AI Opportunities</h3>
              <p className={styles.sectionSub}>
                {myRoles.length} active opportunit{myRoles.length !== 1 ? 'ies' : 'y'} with RYZE —
                recommended builders shown for each
              </p>
            </div>
        ...
        {/* ── Scheduled AI Discovery Calls ── */}
        <section className={styles.section}>
          <div className={styles.sectionHeader}>
            <h3 className={styles.sectionTitle}>AI Discovery Calls</h3>

          </div>

          {bookingsLoading ? (
            <div className={styles.callsEmpty}>Loading your AI discovery calls…</div>
          ) : myBookings.length === 0 ? (
            <div className={styles.callsEmpty}>
              No AI discovery calls scheduled yet.{' '}
              ...
              {myBookings.map(booking => (
                <div key={booking.id} className={styles.callCard}>
                  <div className={styles.callCardLeft}>
                    ...
                    <div className={styles.callMeta}>AI opportunity discovery with RYZE.ai</div>
                  </div>
        ...
        {!rolesLoading && otherRoles.length > 0 && (
          <section className={styles.section}>
            <div className={styles.sectionHeader}>
              <h3 className={styles.sectionTitle}>Other AI Opportunities in the Network</h3>
              <p className={styles.sectionSub}>
                {otherRoles.length} additional opportunit{otherRoles.length !== 1 ? 'ies' : 'y'} being shaped across the RYZE network
              </p>
            </div>
            <div className={styles.jobList}>
              {otherRoles.map(job => (
