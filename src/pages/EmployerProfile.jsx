...
function JobOrderCard({ order }) {
    ...
    const salary = order.salary_min && order.salary_max
        ...
        : order.salary_min
            ? `From $${(order.salary_min / 1000).toFixed(0)}K`
            : null;
        ...
        async function fetchData() {
            try {
                ...
                if (ordersRes.ok) {
                    ...
                    // Filter to job orders for this employer
                ...
                <div className={styles.profileHeaderInner}>
                    ...
                    <div className={styles.headerMain}>
                        ...
                        <div className={styles.headerInfo}>
                            <h1 className={styles.companyName}>{profile.company_name}</h1>
                        ...
                        {jobOrders.length > 0 && (
                            <Section title={`Open Roles (${jobOrders.length})`}>
                                <div className={styles.jobList}>
                                    {jobOrders.map(order => (
                                        <JobOrderCard key={order.id} order={order} />
                                    ))}
                                </div>
                            </Section>
                        )}
                        ...
                        <Section title="Profile Details">
                            <div className={styles.infoList}>
                                {profile.ai_brief_updated_at && (
                                    <InfoRow
                                        label="Brief"
                                        value={new Date(profile.ai_brief_updated_at).toLocaleDateString("en-US", {
                                ...
                                <InfoRow
                                    label="AI Search"
                                    value={profile.embedded_at ? "✓ Indexed" : "Not indexed"}
                                />
                            </div>
                        </Section>
